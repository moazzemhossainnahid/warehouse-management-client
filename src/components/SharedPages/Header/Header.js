...
import VP from '../../../images/vegetable-earth-day-leaf-tree (1).png';
...
const Header = () => {
    ...
    let newLinks = [
        {name: 'Home', to:'/'},
        {name: 'Inventory', to:'/inventory'},
        {name: 'Manage Inventory', to: '/manageinventory'},
        {name: 'Add Inventory', to: '/addinventory'},
        {name: 'Blogs', to:'/blogs'},
        {name: 'About', to:'/about'},
        {name: 'Contact', to:'/contact'},
    ]
        ...
        <nav className='shadow-md w-full fixed top-0 left-0 text-left z-[99]'>
            <div className="md:flex items-center justify-between bg-white py-4 md:px-10 px-7">
                <div onClick={() => {navigate('/')}} className="font-bold text-3xl cursor-pointer flex items-center text-red-700">
                    <span className="px-2 md:px-1"><img src={VP} alt="" className="w-10" /></span>
                    Vegetables Planet
                </div>
                ...
                <ul className={`md:flex md:items-center md:pb-0 pb-12 absolute md:static bg-white md:z-auto z-[-1] left-0 w-full md:w-auto md:pl-0 pl-9 transition-all duration-500 ease-in ${open ? 'top-20 opacity-100' : 'top-[-490px] opacity-0 md:opacity-100'}`}>
                    {
                        user ? newLinks.map(link => (
                            <li key={link.name} className={`md:ml-8 text-sm md:my-0 my-7`}><NavLink className={({ isActive }) => (isActive ? 'text-red-500 duration-300 border-b-2 border-gray-500' : 'text-gray-800 duration-100')} to={link.to}>{link.name}</NavLink></li>
                        ))
                        :Links.map(link => (
